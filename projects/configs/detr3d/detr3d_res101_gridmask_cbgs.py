...
# model firstly constructed base detector without other components(e.g. neck, backbone)
# then, instantiating every components by parent detector MVXTwoStageDetector,也就是递归实例化每个组件,在build_model后，通过递归地实例化每个registry model
# 在build时候，通过传入cfg生成一个对象，后通过forward实现实例化
model = dict(
    type='Detr3D',
    use_grid_mask=True,
    # resnet提取0,1,2,3层的特征
...
train_pipeline = [
    ...
    #  The dict containing loaded 3D bounding box, label, mask and semantic segmentation annotations

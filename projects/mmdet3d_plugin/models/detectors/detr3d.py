...
class Detr3D(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        # parent class is resposible for instantiating every components(e.g. backbone, neck, head...)
    ...
    def forward_pts_train(self,
        ...
        """
        # pts_feats =  [l=4, bs, n, c=256, h, w]
        outs = self.pts_bbox_head(pts_feats, img_metas)
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs]
        losses = self.pts_bbox_head.loss(*loss_inputs)
        return losses
    
    # forward由高层函数调用，默认为return_loss=True,为训练模型,反之在self()传入return_loss为false则为调用forward_test
    ...
    def forward_train(self,
        ...
        Returns:
            ...
            reg,cls的loss
        """
        # backbone generate inter-feature map
        # [num_levels x [B, int(BN / B), C, H, W]]
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        losses = dict()
        # DETR3DHead直接返回损失,关键部分集成在transformer head中
